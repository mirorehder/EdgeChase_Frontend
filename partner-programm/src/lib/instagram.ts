 ...
 *
 * Erwartet werden je Sparte zwei Variablen, z.B. IG_TOKEN_PROMO und
 * IG_USER_ID_PROMO. Ohne einen Satz eigener Werte fällt es auf die allgemeinen
 * IG_TOKEN / IG_USER_ID zurück.
 *
 * Im Partnerprogramm gibt es bereits IG_ACCESS_TOKEN und (per Vorgabe) eine
 * IG_USER_ID für dasselbe EdgeChase-Konto - deshalb ganz zuletzt darauf
 * zurückfallen. So postet der Content-Generator ohne zusätzliche Variablen auf
 * dasselbe Konto wie das Partnerprogramm; wer ein anderes Ziel will, setzt
 * IG_TOKEN / IG_USER_ID (oder die spartengenauen Varianten).
 */
export function igZugang(track: Track): IgZugang | null {
  const suffix = track.toUpperCase();
  const token =
    process.env[`IG_TOKEN_${suffix}`] || process.env.IG_TOKEN || process.env.IG_ACCESS_TOKEN || "";
  const igUserId =
    process.env[`IG_USER_ID_${suffix}`] ||
    process.env.IG_USER_ID ||
    // Vorgabe des Partnerprogramms (EdgeChase-Konto), siehe env.igUserId.
    "17841450788279331";
